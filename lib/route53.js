 ...
 *
 * @param  {string} action uppercase action: UPSERT or DELETE
...
// of records
exports.route53CreatePayload = R.curry(route53Payload)('UPSERT');
 ...
 * @return {[type]}        [description]
 */
const changeResourceRecordSets = async (params) => {
  const res = await route53
    .changeResourceRecordSets(params)
    .promise()

  while((await client.getChange({ Id: res.ChangeInfo.Id }).promise()).ChangeInfo.Status === 'PENDING') {
    console.log('Waiting for DNS change to be processed.')
